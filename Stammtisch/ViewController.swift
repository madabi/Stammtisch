...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, MFMailComposeViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let databaseProgram = getProgramFromDatabase()
        if(databaseProgram != nil){
            self.program = databaseProgram
        }
        self.tableView.reloadData()
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if(self.program.anlaesse.count > 0){
            self.program.save()
            print("saved program")
        }
    ...
    /* Left for next release where editing will be implemented
    ...
    func getProgramFromDatabase() -> ProgramData {
        do {
            let realm = try Realm()
            if(realm.objects(ProgramData.self).last != nil)
                {
                return realm.objects(ProgramData.self).last!
                }else{
                return ProgramData()
            }
